...
export const version = await latestVersion("characters")
...
>
  ...
  <Tab id="kotlin" value="Gradle Kotlin">
    <DynamicCodeBlock lang="kts" code={`implementation("net.thenextlvl:characters:${version}")`} />
  </Tab>
  <Tab id="groovy" value="Gradle Groovy">
    <DynamicCodeBlock
      lang="groovy"
      code={`implementation "net.thenextlvl:characters:${version}"`}
    />
  </Tab>
  <Tab id="sbt" value="SBT">
    <DynamicCodeBlock lang="scala" code={`"net.thenextlvl" %% "characters" %% "${version}"`} />
  </Tab>
</Tabs>
...
The version of the Characters API is the same as the version of the Characters plugin.  
 You can find all versions on the [repository](https://repo.thenextlvl.net/#/releases/net/thenextlvl/characters).
